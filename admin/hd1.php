...
<?
	...
	$sql=mysql_query("select * from hoadon inner join hondonct1 on hoadon.MaHD=hondonct1.MaHD  where TrangthaiHD=$trangthai group by hoadon.MaHD asc limit $start,$display"); 
	...
	?>
 	<tr class="row">
	  ...
	  ?>
        ...
        <td width="8%"><a href="">Chi tiết</a></td>
        <td width="8%"><? if($row['TrangthaiHD']==1) {?><a href="">Xóa</a>
...
<div id="phantrang">
	...
	if($current!=1)
	{
		echo"<li><a href='?go=hd1&status=".$trangthai."&start=$pre'>Pre</a></li>";
	}
	for($i=1; $i<=$page; $i++)
	{
		if($current !=$i)
			{
			echo"<li><a href='?go=hd1&status=".$trangthai."&start=".($display*($i-1))."'>$i</a></li>";
			}
			else
			{
	...
	if($current!=$page)
	{
			echo"<li><a href='?go=hd1&status=".$trangthai."&start=$next'>Next</a></li>";
	}
